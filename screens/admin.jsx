...
function AdminScreen({ onNav, sharedQuestions, setSharedQuestions }) {
  const [items, setItems] = adUseState(sharedQuestions);
  const [editingId, setEditingId] = adUseState(null);
  const [previewIdx, setPreviewIdx] = adUseState(0);
  const [draggingId, setDraggingId] = adUseState(null);
  const [overId, setOverId] = adUseState(null);
