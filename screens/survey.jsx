...
const { useState: svUseState, useEffect: svUseEffect, useRef: svUseRef } = React;

function SurveyScreen({ onNav }) {
  const totalSteps = QUESTIONS.length;
  const [step, setStep] = svUseState(0);
  const [mode, setMode] = svUseState("performance"); // performance vs importance — alternating
  const [answers, setAnswers] = svUseState({});
  const [done, setDone] = svUseState(false);
  const [direction, setDirection] = svUseState(1);
  const stageRef = svUseRef(null);
